...
public interface IDeviceService
{
    ...
    /// 向指定设备推送同步通知
    /// </summary>
    Task<bool> PushSyncToDeviceAsync(PushToVaultRequest request, CancellationToken cancellationToken = default);

    /// <summary>
    /// 获取指定设备的待处理推送请求（移动端长轮询）
    /// </summary>
    Task<IReadOnlyList<PushSyncRequest>> GetPendingPushRequestsAsync(string deviceId, bool wait = false, CancellationToken cancellationToken = default);
