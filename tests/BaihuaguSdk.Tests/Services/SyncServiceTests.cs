...
{
    private readonly Dictionary<string, (HttpStatusCode status, object content)> _responses = new();

    public List<string> RequestLog { get; } = new();

    public void SetupResponse(string path, HttpStatusCode status, object content)
    {
        _responses[path] = (status, content);
    }
    ...
    protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        if (_responses.TryGetValue(path, out var response))
        {
            ...
            return Task.FromResult(new HttpResponseMessage(response.status) { Content = httpContent });
        }
